...
void execute_welch_psd_internal(signal_iq_t* signal_data, const PsdConfig_t* config, double* f_out, double* p_out) {
    ...
    // Generate frequency bins mapped to absolute frequency (centered at f_c)
    double df = fs / nfft;
    double f_c = config->frequency;  // central frequency passed from params
    for (int i = 0; i < nfft; i++) {
        f_out[i] = f_c - fs/2.0 + i * df;
    }
